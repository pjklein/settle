...
import {
  ...
  signSync,
  utils
} from "./chunk-RC5GMUBD.js";
import {
  ChainId,
  TransactionVersion,
  clientFromNetwork,
  networkFrom
} from "./chunk-3ASSPCD5.js";
import {
  __commonJS,
  __require,
  __toESM
...
var require_lib2 = __commonJS({
  "node_modules/@scure/base/lib/index.js"(exports) {
    ...
    var createBase58check = (sha2562) => chain(checksum(4, (data) => sha2562(sha2562(data))), exports.base58);
  ...
  "node_modules/@noble/secp256k1/lib/index.js"(exports) {
    ...
    var JacobianPoint = class _JacobianPoint {
      ...
      double() {
        ...
        const C = mod(B2 * B2);
        const x1b = X1 + B2;
        const D2 = mod(_2n * (mod(x1b * x1b) - A - C));
        const E2 = mod(_3n * A);
        const F2 = mod(E2 * E2);
        const X3 = mod(F2 - _2n * D2);
        const Y3 = mod(E2 * (D2 - X3) - _8n * C);
    ...
    var Point2 = class _Point {
      ...
      toRawBytes(isCompressed2 = false) {
        return hexToBytes6(this.toHex(isCompressed2));
      }
    ...
    var Signature2 = class _Signature {
      ...
      static fromDER(hex) {
        ...
        const { r, s } = parseDERSignature(arr ? hex : hexToBytes6(hex));
    ...
    function numTo32b(num) {
      const b2 = hexToBytes6(numTo32bStr(num));
      if (b2.length !== 32)
    ...
    function bytesToNumber(bytes) {
      return hexToNumber(bytesToHex5(bytes));
    }
    function ensureBytes(hex) {
      return hex instanceof Uint8Array ? Uint8Array.from(hex) : hexToBytes6(hex);
    }
    ...
    function invert(number, modulo = CURVE.P) {
      ...
      let b2 = modulo;
      let x2 = _0n, y2 = _1n, u2 = _1n, v = _0n;
      while (a2 !== _0n) {
        const q2 = b2 / a2;
        const r = b2 % a2;
        const m2 = x2 - u2 * q2;
        const n = y2 - v * q2;
        b2 = a2, a2 = r, x2 = u2, y2 = v, u2 = m2, v = n;
      }
      const gcd = b2;
      if (gcd !== _1n)
    ...
    exports.getPublicKey = getPublicKey3;
    function recoverPublicKey(msgHash, signature, recovery, isCompressed2 = false) {
      return Point2.fromSignature(msgHash, signature, recovery).toRawBytes(isCompressed2);
    }
    ...
    var InternalSchnorrSignature = class {
      ...
      finalizeSig(R, k2, e, d) {
        return new SchnorrSignature(R.x, mod(k2 + e * d, CURVE.n)).toRawBytes();
      }
      error() {
      ...
      async calc() {
        ...
        const { R, rx, k: k2 } = this.finalizeNonce(await tag(TAGS.nonce, t, px, m2));
        const e = schnorrChallengeFinalize(await tag(TAGS.challenge, rx, px, m2));
        const sig = this.finalizeSig(R, k2, e, d);
      ...
      calcSync() {
        ...
        const { R, rx, k: k2 } = this.finalizeNonce(tag(TAGS.nonce, t, px, m2));
    ...
    function finalizeSchnorrVerify(r, P, s, e) {
      const R = Point2.BASE.multiplyAndAddUnsafe(P, normalizePrivateKey(s), mod(-e, CURVE.n));
    ...
    exports.utils = {
      ...
      sha256: async (...messages) => {
        if (crypto2.web) {
          const buffer = await crypto2.web.subtle.digest("SHA-256", concatBytes3(...messages));
...
var require_zone_file_cjs_development = __commonJS({
  "node_modules/zone-file/dist/zone-file.cjs.development.js"(exports) {
    ...
    function removeComments(text) {
      var re2 = /(^|[^\\]);.*/g;
      return text.replace(re2, function(_m, g1) {
        return g1 ? g1 : "";
      });
    }
    ...
    function parseSOA(rr) {
      ...
      var l2 = rrTokens.length;
      soa.name = rrTokens[0];
      soa.minimum = parseInt(rrTokens[l2 - 1], 10);
      soa.expire = parseInt(rrTokens[l2 - 2], 10);
      soa.retry = parseInt(rrTokens[l2 - 3], 10);
      soa.refresh = parseInt(rrTokens[l2 - 4], 10);
      soa.serial = parseInt(rrTokens[l2 - 5], 10);
      soa.rname = rrTokens[l2 - 6];
      soa.mname = rrTokens[l2 - 7];
    ...
    function parseMX(rr) {
      ...
      var result = {
        name: rrTokens[0],
        preference: parseInt(rrTokens[l2 - 2], 10),
        host: rrTokens[l2 - 1]
      };
      if (!isNaN(rrTokens[1])) result.ttl = parseInt(rrTokens[1], 10);
      return result;
    ...
    function parseTXT(rr) {
      ...
      if (l2 - indexTXT - 1 > 1) {
        ...
        tokenTxt = stripText(rrTokens[l2 - 1]);
      }
      var result = {
        name: rrTokens[0],
        txt: tokenTxt
    ...
    function parseSRV(rr) {
      ...
      var result = {
        name: rrTokens[0],
        target: rrTokens[l2 - 1],
        priority: parseInt(rrTokens[l2 - 4], 10),
        weight: parseInt(rrTokens[l2 - 3], 10),
        port: parseInt(rrTokens[l2 - 2], 10)
      };
      if (!isNaN(rrTokens[1])) result.ttl = parseInt(rrTokens[1], 10);
      return result;
    }
    function parseSPF(rr) {
      ...
      var l2 = rrTokens.length;
      while (l2-- > 4) {
        result.data = rrTokens[l2] + " " + result.data.trim();
      }
      if (!isNaN(rrTokens[1])) result.ttl = parseInt(rrTokens[1], 10);
      return result;
    }
    function parseURI(rr) {
      ...
      var result = {
        name: rrTokens[0],
        target: rrTokens[l2 - 1].replace(/"/g, ""),
        priority: parseInt(rrTokens[l2 - 3], 10),
        weight: parseInt(rrTokens[l2 - 2], 10)
      };
      if (!isNaN(rrTokens[1])) result.ttl = parseInt(rrTokens[1], 10);
      return result;
    }
    ...
    (function(global2, factory) {
      ...
      function queue(worker, concurrency, payload) {
        ...
        function _insert(data, insertAtFront, callback) {
          ...
          for (var i = 0, l2 = data.length; i < l2; i++) {
            var item = {
              data: data[i],
              callback: callback || noop
            };
        ...
        function _next(tasks) {
          return function(err) {
            ...
            if (numRunning <= q2.concurrency - q2.buffer) {
              q2.unsaturated();
            }
            if (q2.idle()) {
              q2.drain();
        ...
        var q2 = {
          ...
          process: function() {
            ...
            isProcessing = true;
            while (!q2.paused && numRunning < q2.concurrency && q2._tasks.length) {
              var tasks = [], data = [];
              var l2 = q2._tasks.length;
              if (q2.payload) l2 = Math.min(l2, q2.payload);
              for (var i = 0; i < l2; i++) {
                var node = q2._tasks.shift();
                tasks.push(node);
          ...
          idle: function() {
            return q2._tasks.length + numRunning === 0;
          },
          pause: function() {
            q2.paused = true;
          },
        ...
        var memoized = initialParams(function memoized2(args, callback) {
          ...
          if (has(memo, key)) {
            ...
            _fn.apply(null, args.concat(function() {
              ...
              var q2 = queues[key];
              delete queues[key];
              for (var i = 0, l2 = q2.length; i < l2; i++) {
                q2[i].apply(null, args2);
              }
            }));
          }
        });
      ...
      var priorityQueue = function(worker, concurrency) {
        var q2 = queue$1(worker, concurrency);
        q2.push = function(data, priority, callback) {
          ...
          for (var i = 0, l2 = data.length; i < l2; i++) {
            ...
            if (nextNode) {
              q2._tasks.insertBefore(nextNode, item);
            } else {
              q2._tasks.push(item);
            }
          }
    ...
    (function() {
      ...
      var _validationAttribut = {
        ...
        eq: function(schema, candidate) {
          ...
          if (_typeIs.array(limit)) {
            ...
            this.report("must be equal to [" + limit.map(function(l2) {
              return '"' + l2 + '"';
      ...
      var _formatSample = {
        ...
        color: function(min, max) {
          ...
          for (var i = 0, l2 = _rand.int(min, max); i < l2; i++) {
        ...
        javascript: function(min, max) {
          ...
          for (var i = 0, l2 = _rand.int(min, max - 1); i < l2; i++) {
            s += _rand.fromList("_$abcdefghijklmnopqrstuvwxyz_$ABCDEFGHIJKLMNOPQRSTUVWXYZ_$0123456789_$");
          }
          return s;
        }
      ...
      var _typeGenerator = {
        string: function(schema) {
          ...
          var l2 = schema.exactLength != null ? schema.exactLength : _rand.int(minLength, maxLength);
          for (var i = 0; i < l2; i++) {
            s += _rand.char(32, 126);
          }
          return s;
        ...
        number: function(schema) {
          ...
          if (schema.ne != null) {
            var ne2 = _typeIs.array(schema.ne) ? schema.ne : [schema.ne];
            while (ne2.indexOf(n) !== -1) {
...
var DEFAULT_CORE_NODE = "https://api.hiro.so";
var NAME_LOOKUP_PATH = "/v1/names";
var LOCALSTORAGE_SESSION_KEY = "blockstack-session";

// node_modules/@stacks/auth/node_modules/@stacks/common/dist/esm/config.js
var config = {
  network: {
    layer1: "placeholder"
  },
...
// node_modules/@stacks/auth/node_modules/@stacks/common/dist/esm/errors.js
var ERROR_CODES = {
...
// node_modules/@stacks/auth/node_modules/@stacks/common/dist/esm/logger.js
...
// node_modules/@stacks/auth/node_modules/@stacks/common/dist/esm/utils.js
...
// node_modules/@stacks/auth/node_modules/@stacks/common/dist/esm/constants.js
var HIRO_MAINNET_URL = "https://api.mainnet.hiro.so";
var GAIA_URL = "https://hub.blockstack.org";

// node_modules/@stacks/auth/node_modules/@stacks/common/dist/esm/fetch.js
...
// node_modules/@stacks/encryption/node_modules/@stacks/common/dist/esm/errors.js
var ERROR_CODES2 = {
  MISSING_PARAMETER: "missing_parameter",
  REMOTE_SERVICE_ERROR: "remote_service_error",
  INVALID_STATE: "invalid_state",
...
var BlockstackError2 = class extends Error {
  constructor(error) {
    super();
    let message = error.message;
    let bugDetails = `Error Code: ${error.code}`;
...
var FailedDecryptionError = class extends BlockstackError2 {
  constructor(message = "Unable to decrypt cipher object.") {
    super({ code: ERROR_CODES2.FAILED_DECRYPTION_ERROR, message });
    this.message = message;
    this.name = "FailedDecryptionError";
  }
};

// node_modules/@stacks/encryption/node_modules/@stacks/common/dist/esm/logger.js
...
// node_modules/@stacks/encryption/node_modules/@stacks/common/dist/esm/utils.js
function without0x2(value) {
  return /^0x/i.test(value) ? value.slice(2) : value;
}
...
// node_modules/@stacks/encryption/node_modules/@stacks/common/dist/esm/keys.js
function privateKeyToBytes(privateKey) {
  const privateKeyBuffer = typeof privateKey === "string" ? hexToBytes2(privateKey) : privateKey;
...
function publicKeyToBtcAddress(publicKey, version = BITCOIN_PUBKEYHASH) {
  const publicKeyBytes = typeof publicKey === "string" ? hexToBytes2(publicKey) : publicKey;
...
function getPublicKeyFromPrivate(privateKey) {
  ...
  return bytesToHex2(getPublicKey(privateKeyBytes.slice(0, 32), true));
}
...
async function encryptECIES(publicKey, content, wasString, cipherTextEncoding) {
  ...
  if (!cipherTextEncoding || cipherTextEncoding === "hex") {
    cipherTextString = bytesToHex2(cipherText);
  ...
  const result = {
    iv: bytesToHex2(initializationVector),
    ephemeralPK: bytesToHex2(ephemeralPublicKey),
...
async function decryptECIES(privateKey, cipherObject) {
  ...
  sharedSecret = sharedSecret.slice(1);
  const sharedKeys = sharedSecretToKeys(sharedSecret);
  const ivBytes = hexToBytes2(cipherObject.iv);
  let cipherTextBytes;
  if (!cipherObject.cipherTextEncoding || cipherObject.cipherTextEncoding === "hex") {
    cipherTextBytes = hexToBytes2(cipherObject.cipherText);
  } else if (cipherObject.cipherTextEncoding === "base64") {
    cipherTextBytes = (0, import_base64_js.toByteArray)(cipherObject.cipherText);
  } else {
    throw new Error(`Unexpected cipherTextEncoding "${cipherObject.cipherText}"`);
  }
  const macData = concatBytes(ivBytes, hexToBytes2(ephemeralPK), cipherTextBytes);
  const actualMac = hmacSha256(sharedKeys.hmacKey, macData);
  const expectedMac = hexToBytes2(cipherObject.mac);
...
var import_jsontokens = __toESM(require_lib4());
...
var import_jsontokens2 = __toESM(require_lib4());
function getAuthRequestFromURL() {
  ...
  return ((_a = params.get("authRequest")) == null ? void 0 : _a.replaceAll(`${BLOCKSTACK_HANDLER2}:`, "")) ?? null;
}
...
var import_jsontokens3 = __toESM(require_lib4());
...
var import_jsontokens5 = __toESM(require_lib4());

// node_modules/@stacks/profile/dist/esm/profileTokens.js
var import_jsontokens4 = __toESM(require_lib4());

// node_modules/@stacks/profile/node_modules/@stacks/common/dist/esm/config.js
var config3 = {
  network: {
    layer1: "placeholder"
  },
...
// node_modules/@stacks/profile/node_modules/@stacks/common/dist/esm/errors.js
var ERROR_CODES3 = {
  MISSING_PARAMETER: "missing_parameter",
  REMOTE_SERVICE_ERROR: "remote_service_error",
  INVALID_STATE: "invalid_state",
...
Object.freeze(ERROR_CODES3);

// node_modules/@stacks/profile/node_modules/@stacks/common/dist/esm/logger.js
var levels3 = ["debug", "info", "warn", "error", "none"];
var levelToInt3 = {};
var intToLevel3 = {};
for (let index = 0; index < levels3.length; index++) {
  const level = levels3[index];
  levelToInt3[level] = index;
  intToLevel3[index] = level;
}
var Logger3 = class {
  static error(message) {
    if (!this.shouldLog("error"))
  ...
  static shouldLog(level) {
    const currentLevel = levelToInt3[config3.logLevel];
    return currentLevel <= levelToInt3[level];
  }
};

// node_modules/@stacks/profile/node_modules/@stacks/common/dist/esm/utils.js
function nextYear() {
...
function resolveZoneFileToProfile(opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      zoneFileJson = (0, import_zone_file2.parseZoneFile)(opts.zoneFile);
    ...
    if (tokenFileUrl) {
      client.fetch(tokenFileUrl).then((response) => response.text()).then((responseText) => JSON.parse(responseText)).then((responseJson) => {
        const tokenRecords = responseJson;
        const profile = extractProfile(tokenRecords[0].token, opts.publicKeyOrAddress);
        resolve(profile);
      }).catch((error) => {
        Logger3.error(`resolveZoneFileToProfile: error fetching token file ${tokenFileUrl}: ${error}`);
        reject(error);
      });
    } else {
      Logger3.debug("Token file url not found. Resolving to blank profile.");
      resolve({});
    }
  });
}
...
var import_jsontokens6 = __toESM(require_lib4(), 1);
...
Object.freeze(ERROR_CODES4);

// node_modules/@stacks/common/dist/esm/logger.js
var levels4 = ["debug", "info", "warn", "error", "none"];
var levelToInt4 = {};
var intToLevel4 = {};
for (let index = 0; index < levels4.length; index++) {
  const level = levels4[index];
  levelToInt4[level] = index;
  intToLevel4[index] = level;
}

// node_modules/@stacks/common/dist/esm/utils.js
function intToBytes(value, signed, byteLength) {
  return bigIntToBytes2(intToBigInt(value, signed), byteLength);
}
function intToBigInt(value, signed) {
  ...
  if (parsedValue instanceof Uint8Array) {
    if (signed) {
      const bn2 = fromTwos(BigInt(`0x${bytesToHex3(parsedValue)}`), BigInt(parsedValue.byteLength * 8));
      return BigInt(bn2.toString());
    } else {
...
function bigIntToBytes2(value, length = 16) {
...
function concatArray(elements) {
  return concatBytes2(...elements.map((e) => {
    if (typeof e === "number")
...
var StacksNetwork = class {
  constructor(networkConfig) {
    ...
    this.fetchFn = networkConfig.fetchFn ?? createFetchFn3();
  }
...
// node_modules/@stacks/connect/node_modules/@stacks/common/dist/esm/errors.js
var ERROR_CODES5 = {
  MISSING_PARAMETER: "missing_parameter",
  REMOTE_SERVICE_ERROR: "remote_service_error",
  INVALID_STATE: "invalid_state",
...
Object.freeze(ERROR_CODES5);

// node_modules/@stacks/connect/node_modules/@stacks/common/dist/esm/logger.js
var levels5 = ["debug", "info", "warn", "error", "none"];
var levelToInt5 = {};
var intToLevel5 = {};
for (let index = 0; index < levels5.length; index++) {
  const level = levels5[index];
  levelToInt5[level] = index;
  intToLevel5[index] = level;
}

// node_modules/@stacks/connect/node_modules/@stacks/common/dist/esm/utils.js
function without0x3(value) {
  return /^0x/i.test(value) ? value.slice(2) : value;
}
...
var DEFAULT_TRANSACTION_VERSION = TransactionVersion3.Mainnet;
...
function serializeAddress(address) {
  const bytesArray = [];
  bytesArray.push(hexToBytes4(intToHex(address.version, 1)));
  bytesArray.push(hexToBytes4(address.hash160));
...
function serializeLPString(lps) {
  ...
  bytesArray.push(hexToBytes4(intToHex(length, lps.lengthPrefixBytes)));
...
function serializeOptionalCV(cv) {
  if (cv.type === ClarityType.OptionalNone) {
    ...
    return bytesWithTypeID(cv.type, serializeCV2(cv.value));
  }
}
function serializeBufferCV(cv) {
  const length = new Uint8Array(4);
  writeUInt32BE2(length, cv.buffer.length, 0);
  return bytesWithTypeID(cv.type, concatBytes2(length, cv.buffer));
}
function serializeIntCV(cv) {
  const bytes = bigIntToBytes2(toTwos(cv.value, BigInt(CLARITY_INT_SIZE)), CLARITY_INT_BYTE_SIZE);
  return bytesWithTypeID(cv.type, bytes);
}
function serializeUIntCV(cv) {
  const bytes = bigIntToBytes2(cv.value, CLARITY_INT_BYTE_SIZE);
  return bytesWithTypeID(cv.type, bytes);
}
...
function serializeContractPrincipalCV(cv) {
  return bytesWithTypeID(cv.type, concatBytes2(serializeAddress(cv.address), serializeLPString(cv.contractName)));
}
...
function serializeListCV(cv) {
  const bytesArray = [];
  const length = new Uint8Array(4);
  writeUInt32BE2(length, cv.list.length, 0);
...
function serializeTupleCV(cv) {
  const bytesArray = [];
  const length = new Uint8Array(4);
  writeUInt32BE2(length, Object.keys(cv.data).length, 0);
  bytesArray.push(length);
...
function serializeStringCV(cv, encoding) {
  const bytesArray = [];
  const str = encoding == "ascii" ? asciiToBytes(cv.data) : utf8ToBytes3(cv.data);
...
// node_modules/@stacks/transactions-v6/dist/esm/utils.js
var exceedsMaxLengthBytes = (string, maxLengthBytes) => string ? utf8ToBytes3(string).length > maxLengthBytes : false;
...
var defaultFetchOpts3 = {
  referrerPolicy: "origin",
  headers: {
    "x-hiro-product": "stacksjs"
  }
};
async function fetchWrapper3(input, init) {
  const fetchOpts = {};
  Object.assign(fetchOpts, defaultFetchOpts3, init);
  const fetchResult = await fetch(input, fetchOpts);
  return fetchResult;
}
function argsForCreateFetchFn3(args) {
  let fetchLib = fetchWrapper3;
  let middlewares = [];
  if (args.length > 0 && typeof args[0] === "function") {
    fetchLib = args.shift();
  }
...
function createFetchFn4(...args) {
  const { fetchLib, middlewares } = argsForCreateFetchFn3(args);
  const fetchFn = async (url, init) => {
    let fetchParams = { url, init: init ?? {} };
    for (const middleware of middlewares) {
      if (typeof middleware.pre === "function") {
...
var StacksNetwork2 = class {
  constructor(networkConfig) {
    ...
    this.getMapEntryUrl = (contractAddress, contractName, mapName) => `${this.coreApiUrl}/v2/map_entry/${contractAddress}/${contractName}/${mapName}?proof=0`;
    this.coreApiUrl = networkConfig.url;
    this.fetchFn = networkConfig.fetchFn ?? createFetchFn4();
  }
  getNameInfo(fullyQualifiedName) {
    const nameLookupURL = `${this.bnsLookupUrl}/v1/names/${fullyQualifiedName}`;
    return this.fetchFn(nameLookupURL).then((resp) => {
...
var O = (e, t, n) => t in e ? rt(e, t, { enumerable: true, configurable: true, writable: true, value: n }) : e[t] = n;
var a = (e, t) => {
  for (var n in t || (t = {})) k.call(t, n) && O(e, n, t[n]);
  if (x) for (var n of x(t)) E.call(t, n) && O(e, n, t[n]);
  return e;
};
var u = (e, t) => st(e, at(t));
var g = (e, t) => {
  var n = {};
  for (var o in e) k.call(e, o) && t.indexOf(o) < 0 && (n[o] = e[o]);
  if (e != null && x) for (var o of x(e)) t.indexOf(o) < 0 && E.call(e, o) && (n[o] = e[o]);
  return n;
};
function l() {
...
function y(e) {
  return e ? typeof e == "string" ? StacksNetwork.fromName(e) : "version" in e ? e : "url" in e ? new StacksMainnet({ url: e.url }) : e.transactionVersion === TransactionVersion.Mainnet ? new StacksMainnet({ url: e.client.baseUrl }) : new StacksTestnet({ url: e.client.baseUrl }) : new StacksTestnet();
}
var Te = "https://app.blockstack.org";
...
var U = async (e, t = l()) => {
  ...
  let { redirectTo: n = "/", manifestPath: o, onFinish: s, onCancel: r, sendToSignIn: i = false, userSession: d, appDetails: p } = e, c = b(d);
  c.isUserSignedIn() && c.signUserOut();
  let C = c.generateAndStoreTransitKey(), T = c.makeAuthRequest(C, `${document.location.origin}${n}`, `${document.location.origin}${o}`, c.appConfig.scopes, void 0, void 0, { sendToSignIn: i, appDetails: p, connectVersion: "__VERSION__" });
  try {
    let j = await t.authenticationRequest(T);
    await c.handlePendingSignIn(j);
    let A = (0, import_jsontokens6.decodeToken)(j), ot = A == null ? void 0 : A.payload;
    s == null || s({ authResponse: j, authResponsePayload: ot, userSession: c });
  } catch (j) {
    console.error("[Connect] Error during auth request", j), r == null || r();
  }
};
var fe = async (e) => (e = b(e), e.isUserSignedIn() ? e.loadUserData() : e.isSignInPending() ? e.handlePendingSignIn() : null);
var St = ((o) => (o.ContractCall = "contract_call", o.ContractDeploy = "smart_contract", o.STXTransfer = "token_transfer", o))(St || {});
var yt = ((r) => (r.BUFFER = "buffer", r.UINT = "uint", r.INT = "int", r.PRINCIPAL = "principal", r.BOOL = "bool", r))(yt || {});
var D = (e) => {
  let t = e;
  if (!t) {
...
function M(e) {
  try {
    return D(e).loadUserData().appPrivateKey;
  } catch (t) {
    return false;
  }
}
var S = (e) => {
  let n = D(e).loadUserData().appPrivateKey, o = import_jsontokens6.SECP256K1Client.derivePublicKey(n);
  return { privateKey: n, publicKey: o };
};
function wt(e) {
  var p;
  let { stxAddress: t, userSession: n, network: o } = e;
  if (t) return t;
  if (!n || !o) return;
  let s = (p = n == null ? void 0 : n.loadUserData().profile) == null ? void 0 : p.stxAddress, r = { [ChainId.Mainnet]: "mainnet", [ChainId.Testnet]: "testnet" }, i = y(o);
  return s == null ? void 0 : s[r[i.chainId]];
}
function Nt(e) {
  let t = y(e.network), n = D(e.userSession), o = u(a({}, e), { network: t, userSession: n });
  return a({ stxAddress: wt(o) }, o);
}
async function f(e, t) {
  let { postConditions: n } = e;
  return n && n.length > 0 && typeof n[0] != "string" && (typeof n[0].type == "string" ? n = n.map(postConditionToHex) : n = n.map((s) => bytesToHex4(serializePostCondition(s)))), new import_jsontokens6.TokenSigner("ES256k", t).signAsync(u(a({}, e), { postConditions: n }));
}
function w(e) {
  let { postConditions: t } = e;
  return t && t.length > 0 && typeof t[0] != "string" && (typeof t[0].type == "string" ? t = t.map(postConditionToHex) : t = t.map((n) => bytesToHex4(serializePostCondition(n)))), (0, import_jsontokens6.createUnsecuredToken)(u(a({}, e), { postConditions: t }));
}
var Pt = async ({ token: e, options: t }, n) => {
  var o, s, r;
  try {
    let i = await n.transactionRequest(e), { txRaw: d } = i, p = hexToBytes5(d.replace(/^0x/, "")), c = deserializeTransaction(p);
    if ("sponsored" in t && t.sponsored) {
      (o = t.onFinish) == null || o.call(t, u(a({}, i), { stacksTransaction: c }));
      return;
    }
    (s = t.onFinish) == null || s.call(t, u(a({}, i), { stacksTransaction: c }));
  } catch (i) {
    console.error("[Connect] Error during transaction request", i), (r = t.onCancel) == null || r.call(t);
  }
};
var mt = async (e) => {
  let d = e, { functionArgs: t, appDetails: n, userSession: o } = d, s = g(d, ["functionArgs", "appDetails", "userSession"]), r = t.map((p) => typeof p == "string" ? p : typeof p.type == "string" ? serializeCV(p) : bytesToHex4(serializeCV2(p)));
  if (M(o)) {
    let { privateKey: p, publicKey: c } = S(o), C = u(a({}, s), { functionArgs: r, txType: "contract_call", publicKey: c });
    return n && (C.appDetails = n), f(C, p);
  }
  let i = u(a({}, s), { functionArgs: r, txType: "contract_call" });
  return n && (i.appDetails = n), w(i);
};
var Ot = async (e) => {
  let r = e, { appDetails: t, userSession: n } = r, o = g(r, ["appDetails", "userSession"]);
  if (M(n)) {
    let { privateKey: i, publicKey: d } = S(n), p = u(a({}, o), { publicKey: d, txType: "smart_contract" });
    return t && (p.appDetails = t), f(p, i);
  }
  let s = u(a({}, o), { txType: "smart_contract" });
  return t && (s.appDetails = t), w(s);
};
var kt = async (e) => {
  let i = e, { amount: t, appDetails: n, userSession: o } = i, s = g(i, ["amount", "appDetails", "userSession"]);
  if (M(o)) {
    let { privateKey: d, publicKey: p } = S(o), c = u(a({}, s), { amount: t.toString(10), publicKey: p, txType: "token_transfer" });
    return n && (c.appDetails = n), f(c, d);
  }
  let r = u(a({}, s), { amount: t.toString(10), txType: "token_transfer" });
  return n && (r.appDetails = n), w(r);
};
var Et = async (e) => {
  let i = e, { txHex: t, appDetails: n, userSession: o } = i, s = g(i, ["txHex", "appDetails", "userSession"]);
  if (M(o)) {
    let { privateKey: d, publicKey: p } = S(o), c = u(a({}, s), { txHex: t, publicKey: p });
    return n && (c.appDetails = n), f(c, d);
  }
  let r = u(a({}, s), { txHex: t });
  return n && (r.appDetails = n), w(r);
...
async function N(e, t, n) {
  let o = await t(u(a(a({}, Nt(e)), e), { network: y(e.network) }));
  return Pt({ token: o, options: e }, n);
}
function Y(e, t = l()) {
  if (!t) throw new Error("[Connect] No installed Stacks wallet found");
  return N(e, mt, t);
...
async function vt({ token: e, options: t }, n) {
  var o, s;
  if (!n) throw new Error("[Connect] No installed Stacks wallet found");
  try {
    let r = await n.psbtRequest(e);
    (o = t.onFinish) == null || o.call(t, r);
  } catch (r) {
    console.error("[Connect] Error during psbt request", r), (s = t.onCancel) == null || s.call(t);
  }
}
var Rt = async (e) => {
  let d = e, { allowedSighash: t, hex: n, signAtIndex: o, userSession: s } = d, r = g(d, ["allowedSighash", "hex", "signAtIndex", "userSession"]);
  if (M(s)) {
    let { privateKey: p, publicKey: c } = S(s), C = u(a({}, r), { allowedSighash: t, hex: n, signAtIndex: o, publicKey: c });
    return bt(C, p);
  }
  let i = a({}, r);
...
async function Qt(e, t, n) {
  let o = await t(a(a({}, Ut(e)), e));
  return vt({ token: o, options: e }, n);
}
function F(e, t = l()) {
  return Qt(e, Rt, t);
}
function Gt(e) {
  ...
  if (!t || !n) return;
  let o = (d = t == null ? void 0 : t.loadUserData().profile) == null ? void 0 : d.stxAddress, s = { [ChainId.Mainnet]: "mainnet", [ChainId.Testnet]: "testnet" }, r = y(n);
  return o == null ? void 0 : o[s[r.chainId]];
}
...
function m(e) {
  let t = y(e.network), n = D(e.userSession), o = u(a({}, e), { network: t, userSession: n });
  return a({ stxAddress: Gt(o) }, o);
}
async function Ft({ token: e, options: t }, n) {
  var o, s;
  try {
    let r = await n.signatureRequest(e);
    (o = t.onFinish) == null || o.call(t, r);
  } catch (r) {
    console.error("[Connect] Error during signature request", r), (s = t.onCancel) == null || s.call(t);
  }
}
var Zt = async (e) => {
  let s = e, { userSession: t } = s, n = g(s, ["userSession"]);
  if (M(t)) {
    let { privateKey: r, publicKey: i } = S(t), d = u(a({}, n), { publicKey: i });
    return qt(d, r);
  }
  let o = a({}, n);
  return (0, import_jsontokens6.createUnsecuredToken)(o);
};
async function Wt(e, t, n) {
  let o = await t(a(a({}, m(e)), e));
  return Ft({ token: o, options: e }, n);
}
function W(e, t = l()) {
  if (!t) throw new Error("[Connect] No installed Stacks wallet found");
  return Wt(e, Zt, t);
...
async function Kt(e, t, n) {
  let o = await t(a(a({}, m(e)), e));
  return Ht({ token: o, options: e }, n);
}
function _(e) {
  let { message: t, domain: n } = e;
  return typeof t.type == "string" && typeof n.type == "string" ? u(a({}, e), { message: serializeCV(t), domain: serializeCV(n) }) : u(a({}, e), { message: bytesToHex4(serializeCV2(t)), domain: bytesToHex4(serializeCV2(n)) });
}
async function _t(e, t) {
  return new import_jsontokens6.TokenSigner("ES256k", t).signAsync(_(e));
}
async function Jt(e) {
  let o = e, { userSession: t } = o, n = g(o, ["userSession"]);
  if (M(t)) {
    let { privateKey: s, publicKey: r } = S(t), i = u(a({}, n), { publicKey: r });
    return _t(i, s);
  }
  return (0, import_jsontokens6.createUnsecuredToken)(_(e));
}
async function Ht({ token: e, options: t }, n) {
  var o, s;
  try {
    let r = await n.structuredDataSignatureRequest(e);
    (o = t.onFinish) == null || o.call(t, r);
  } catch (r) {
    console.error("[Connect] Error during signature request", r), (s = t.onCancel) == null || s.call(t);
  }
...
async function oe({ token: e, options: t }, n) {
  var o, s;
  try {
    let r = await n.profileUpdateRequest(e);
    (o = t.onFinish) == null || o.call(t, r);
  } catch (r) {
    console.error("[Connect] Error during signature request", r), (s = t.onCancel) == null || s.call(t);
  }
...
var re = async (e) => {
  let r = e, { userSession: t, profile: n } = r, o = g(r, ["userSession", "profile"]);
  if (M(t)) {
    let { privateKey: i, publicKey: d } = S(t), p = u(a({}, o), { profile: n, publicKey: d });
    return ee(p, i);
  }
  let s = a({}, o);
...
var Rn = I(W);
var Qn = I(J);
var Yn = clearSelectedProviderId;
var Bn = ge;
export {
  AppConfig,
  ...
  yt as ContractCallArgumentType,
  DEFAULT_BLOCKSTACK_HOST,
  DEFAULT_CORE_NODE,
  DEFAULT_PROFILE,
  nt as DEFAULT_PROVIDERS,
  DEFAULT_SCOPE,
  LOCALSTORAGE_SESSION_KEY,
  NAME_LOOKUP_PATH,
  ae as SignatureHash,
  St as TransactionTypes,
  UserSession,
  U as authenticate,
  clearSelectedProviderId,
  decryptPrivateKey,
  Te as defaultAuthURL,
  Yn as disconnect,
  ...
  getDIDType,
  ne as getDefaultProfileUpdateRequestOptions,
  Ut as getDefaultPsbtRequestOptions,
  m as getDefaultSignatureRequestOptions,
  S as getKeys,
  getNameInfo,
  b as getOrCreateUserSession,
  getSelectedProviderId,
  l as getStacksProvider,
  wt as getStxAddress,
  fe as getUserData,
  D as getUserSession,
  M as hasAppPrivateKey,
  isExpirationDateValid,
  isIssuanceDateValid,
  isManifestUriValid,
  Mt as isMobile,
  isRedirectUriValid,
  Ie as isStacksWalletInstalled,
  y as legacyNetworkFromConnectNetwork,
  lookupProfile,
  makeAuthRequest,
  makeAuthRequestToken,
  makeAuthResponse,
  mt as makeContractCallToken,
  Ot as makeContractDeployToken,
  makeDIDFromAddress,
  makeDIDFromPublicKey,
  re as makeProfileUpdateToken,
  Rt as makePsbtToken,
  kt as makeSTXTransferToken,
  Et as makeSignTransaction,
  Y as openContractCall,
  B as openContractDeploy,
  H as openProfileUpdateRequestPopup,
  F as openPsbtRequestPopup,
  G as openSTXTransfer,
  q as openSignTransaction,
  W as openSignatureRequestPopup,
  J as openStructuredDataSignatureRequestPopup,
  setSelectedProviderId,
  xe as shouldUsePopup,
  Bn as showBlockstackConnect,
  ge as showConnect,
  hn as showContractCall,
  zn as showContractDeploy,
  vn as showProfileUpdate,
  Un as showPsbt,
  En as showSTXTransfer,
  Rn as showSignMessage,
  Qn as showSignStructuredMessage,
  bn as showSignTransaction,
  Zt as signMessage,
  Jt as signStructuredMessage,
  verifyAuthRequest,
