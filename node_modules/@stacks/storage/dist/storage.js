...
class Storage {
    ...
    async getFileSignedUnencrypted(path, opt) {
        ...
        try {
            ...
            const signerAddress = (0, encryption_1.publicKeyToBtcAddress)(publicKey);
            if (gaiaAddress !== signerAddress)
                throw new common_1.SignatureVerificationError(`Signer pubkey address (${signerAddress}) doesn't match gaia address (${gaiaAddress})`);
            if (!(0, encryption_1.verifyECDSA)(typeof fileContents === 'string'
                ? (0, common_1.utf8ToBytes)(fileContents)
                : new Uint8Array(fileContents), publicKey, signature)) {
                throw new common_1.SignatureVerificationError(`Contents do not match ECDSA signature: path: ${path}, signature: ${path}${SIGNATURE_FILE_SUFFIX}`);
            }
            return fileContents;
        }
        catch (err) {
            if (err instanceof common_1.DoesNotExist && err.message.indexOf(sigPath) >= 0)
                throw new common_1.SignatureVerificationError(`Failed to obtain signature for file: ${path} -- looked in ${path}${SIGNATURE_FILE_SUFFIX}`);
            throw err;
        }
    }
    async handleSignedEncryptedContents(path, storedContents, app, privateKey, username, zoneFileLookupURL) {
        ...
        const address = username
            ...
            : (0, encryption_1.publicKeyToBtcAddress)(appPublicKey);
        if (!address) {
            throw new common_1.SignatureVerificationError(`Failed to get gaia address for verification of: ${path}`);
        }
        let sigObject;
        ...
        const signerAddress = (0, encryption_1.publicKeyToBtcAddress)(signerPublicKey);
